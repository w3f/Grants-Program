...
### Mildestone 2: Staking-Mining module, Distribution Module, and their Front-End
...
| 3.    | Substrate Module: Distribution HPT  | To secure a parachain slot, we run a crowdfunding for Dot. User deposit Dot to help HexPlay for the auction, and, if we succeed,  as a reward user will receive HPT. The module basically realizes the functionality (1) snapshot function: record which user deposits how much Dot,  (2)sendReward function, every sometime module sends the reward to stakers according to how much Dot they stake, (3) claimReward: stakers claim HPT rewarded to their accounts. Plus there are other functions that enable a user to query how much reward is available by now, etc. The exact way by which the HPT is distributed will be designed and written into the logic of sendReward function.|
