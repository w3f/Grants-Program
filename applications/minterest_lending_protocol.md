...
Minterest is a lending protocol built on Rust and Substrate. The Protocol connects to asset providers on blockchain networks and allows the supplying or borrowing of these assets. The Minterest Protocol enables the creation of token markets with pools of token assets. Each pool algorithmically determines interest rates to actuate borrowing prices based on the supply and demand of the particular asset. Possible accrued interest incentivizes users to balance the pool state.

Lenders and borrowers of a token asset interact directly with the protocol, earning and paying interest. The rates float as demand and supply dynamics vary, similarly to the way traditional forex markets do, without users having to individually manage aspects of lending, such as settlement, credit checks, maturity dates, interest rates or lodging collateral with a trusted third party.

Each token market is unique to the token asset, whether on the Ethereum, Polkadot or Bitcoin networks and includes tokens such as Ether, US dollar-pegged stablecoins like Dai, or Bitcoin itself. Each market implements a unique and tweakable economic model to balance the pool. Adding new pools on demand requires minor changes in the Protocol code, especially if a bridge application is available. Given the decentralized nature of the protocol, each token market contains a transparent, non-violable and publicly available immutable ledger which records each transaction and the applied interest rates.
...
- Allow cheaper transactions. Compound, Aave and Maker (three key players on the DeFi market) utilise Ethereum, where Gas price has become an impediment to trading due to extreme Gas price increases.
...
- Implement an advanced economic environment, which incentivizes users to keep the assets in the pool (using SWAPs and automated liquidation, where the protocol itself plays the role of liquidator).

The Protocol aims to become decentralized and self-sustaining in one or two years. The Protocol issues governance tokens (called MNT token) as a reward for transactions undertaken by users. The members may then utilise their right to vote and trigger voting to decide the future direction of the Protocol. In the end, the platform will become independent from the Minterest Labs OU and fully decentralized.
...
All the endpoints require amount + asset type + balance id to operate on as incoming variables, but actual implementation will differ for each asset type and heavily depend on connected bridge logic.

The incoming asset is locked inside a specific Liquidity pool (depending on the asset type), and the Protocol mints wrap tokens (called mTokens - for accounting purposes) to the user balance. mTokens are ERC-20 standardised tokens. From now on and till the asset is withdrawn from the pool, the Protocol operates with these wrap tokens (mTokens)- accrues interest, and uses the liquidity as Collateral for borrowing etc. In addition, the Protocol rewards users with an amount of governance tokens for lending and borrowing.

The governance token (MNT token) can be utilised to vote for or suggest new Protocol changes via proposals.
...
First group are the applications which provide their own stablecoin. Example, [Laminar](https://github.com/laminar-protocol/laminar-chain) and specifically the Flow and the Collateralized Synthetic Asset protocols are designed to turn fiat funds into synthetic tokens - fTokens. [Acala](https://github.com/AcalaNetwork/Acala) mints aUSD in exchange for different types of assets. [Centrifuge](https://github.com/centrifuge/) allows turning real-world assets like mortgages into stablecoins. [Stafi](https://docs.stafi.io/) wraps stacked assets into a tradable form (rToken) - enabling both getting interest from the main-net and trading at the same time. [Bandot](https://github.com/bandotorg/Bandot) transforms assets to stablecoin. Same goes for [Equilibrium](https://github.com/equilibrium-eosdt) - which has a credit market and its own stablecoin. 

Minterest protocol will create financial markets for stablecoins like these through its facilitation of lending and borrowing of stablecoin token assets. Users of the protocol will therefore have added tokens for yield farming and accruing interest and this activity supports liquidity for such stablecoins.
...
Last but not least, we are building the Protocol on the basis of valueing an easier user experience. Our aim is to make DeFi available for average person with close to no knowledge about crypto assets and how to utlise them.
...
- [Adam Gagol](https://www.linkedin.com/in/adam-gągol-b07904154/)
...
We are in the process of deploying a test staging environment.
...
### Milestone 1 — Implement functional economic model on Substrate.
...
| 0d.    | Article/Tutorial                 | The team is ready to write an article describing the basic economy implemented to verify the work done for the grant milestone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 3.     | Substrate module: User Balance   | This module is responsible for storing wrap token receipts of users (both lend and borrow receipts - utlising mtokens to do this). It is based on [ORML](https://github.com/open-web3-stack/open-runtime-module-library) implementation of ERC-20 token and allows to keep track of current user balance across the pools, transfer tokens between users.                                                                                                                                                                                                                                                                                                                             |
| 4.     | Substrate module: Admin Panel    | The Protocol should be maintained by a set of admin users. This module gives a toolset for tweaking the economy models of different pools, enabling or disabling operations, managing admins list, treasuries etc.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 5.     | React-base UI                    | This is a front end application, developed for users to be able to interact with the Protocol. At this point it emulates a wallet-based working interface and allows to call all supported extrinsics as both an admin and non-admin user. No fancy UX yet though.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 6.     | Substrate chain                  | The chain supports calls for 4 main operations - lend assets, withdraw assets, borrow and pay out assets. Each call goes to a Controller, which first validates it and updates all current rates and accrues interest for the signed user. This is done invoking User Balance and Liquidity Pool modules. When the update is done, the Controller executes the required functions on the Pool and User Balance to store incoming changes - adding or removing liquidity from the protocol. The Admin Panel stays a bit aside as contains mostly independent Extrinsics, which allows the storing of flags and values governing the economy of a specific Liquidity pool. |
...
| 0c.    | Testing Guide                | The test coverage is 100% (both unit and integration tests). The Make file has a command to run them. The tests cover the distribution logic of the MNT token (governance token) between lenders and borrowers. The oracles pallet is covered with tests while network connections are stubbed. The economy tests are tweaked to work with the stubbed Oracles pallet (based on [ORML](https://github.com/open-web3-stack/open-runtime-module-library/tree/master/oracle)). |
...
| 4.     | Substrate chain              | The chain should now do two more things - take aggregated prices from the outside world to be used in calculations and also distribute MNT tokens for between users.                                                                                                                                                                                          |
| 5.     | Docker                       | We will provide a dockerfile to demonstrate the full functionality of our chain                                                                                                                                                                                                                                                                              |

## Future Plans

Current plan is to go Live in July-August 2021 with working integrations to BTC, Ethtereum and Polkadot networks, a complex Oracles system and a reasonable set of supported wallets.
Further work will be aimed at enlisting more popular assets (including those which are not yet public) in the Protocol and also providing more tools for trading, such as SWAPs.
One of the priorities is to make the Protocol fully independent in the end.

A short roadmap of further Protocol development:
...
We are negotiating with Plasm and Moonbeam as part of our efforts in finding bridges to other networks.
