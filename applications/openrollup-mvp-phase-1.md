...
- **Zkapp registration.** Developers use program-hash to register a zkapp. Program-hash is derived from the bytecode of zkapp. If the zkapp is open source, users can verify that the program-hash is indeed corresponding to the zkapp. In addition, program-hash is also a public output of the verifier, which ensures that the submitted proof is indeed generated by this zkapp's execution.
...
- **program_hash** all zkapp can be reduced to a single 32-byte value, called program hash, it ensures that the verifier verifies execution of a specific program
...
Our team  have much experience in applied cryptography, blockchain research, and software development. 
