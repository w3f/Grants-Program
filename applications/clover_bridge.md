...
#### Introduction
The Clover Bridge is a threshold signature based protocol for enabling cross-chain communication between Polkadot based networks and non-Polkadot based networks. We propose an end-2-end secure communication protocol between signers for notarising block headers from Polkadot based parachains. Notarised block headers along with a patricia merkle path, generate proof of inclusion for transactions that can be verified outside the Polkadot context. By utilizing using Schnorr signatures, Clover Bridge can communicate with Turing complete-blockchains like Ethereum, and non-turing complete blockchains like Bitcoin with the upcoming BIP-SChnorr upgrade.
