...
The communication between the signers are done through gRPC services with mutual SSL/TLS protocol. This protocol will only work with the certificates generated by the same internal Certificate Authority (CA). Eliptic curve notation is used for the discrete logarithm problem, plus a verifiable Secret Sharing scheme (VSS) to prevent dealers from cheating where each participany can verify their own share.
