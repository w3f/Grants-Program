...
Will describe the left EasyPolka in the furture plan part.
...
Will try to apply the left part, it is a bit complex, I am working on regrouping them now.

### Project Details

- Anchor development itself has been done, and it is deployed here [wss://dev.metanchor.net](wss://dev.metanchor.net). One cApp is deployed to test too, the URL is [https://freesaying.net](https://freesaying.net). This React project load the target Anchor "freeSaying" from dev.metanchor.net, then run the cApp which is a socail media application.
...
- Code here [https://github.com/ff13dfly/Anchor], the document is the next step.

- If treat Anchor as Name Service, there is no available pallet can be use. Have checked here [Open Source Polkadot Stack](https://wiki.polkadot.network/docs/build-open-source).
...
- You can develop and apply flexibly without understanding the complete blockchain system.

- From the [Open Source Polkadot Stack](https://wiki.polkadot.network/docs/build-open-source), there are 4 Name Service pallet, 2 red ( can not compatible to substrate 3.0.0 ) and 2 yellow ( code not updated more than 2 months by checking github ).
...
- Demo cApp [freeSaying](https://android.im/vManager/) is published now. The test network is available, you can access [wss:dev.metanchor.net](wss:dev.metanchor.net). Anchor pallet functions have been developed, but not tested entirely.
...
- I have tried twice to apply the Gant Application but not accepted.
  ...
  [SimPolk](), simulator for Polkadot.
