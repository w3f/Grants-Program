...
| 3. | Secret Management | The previous `systemd unit` will be granted access to the validator secrets with the combined usage of [`systemd-vault`](https://github.com/numtide/systemd-vaultd) and Hashicorp's [`vault`](https://www.vaultproject.io/). `systemd units` have the option to use the `LoadCredential=`  to provide access to secrets from `vault` to the `systemd unit`.<br />Additonal Details:<br />- configure vault using [integrated storage](https://developer.hashicorp.com/vault/docs/internals/integrated-storage)<br />- configure `vault-agent` to integrate with `systemd-vault` i.e. by writing secrets to `/run/systemd-vaultd/secrets/$service_name.service.json`<br />- configure `systemd-vaultd`<br />- configure the validator to load credentials from systemd-vault<br />- restart the validator whenever secrets change |
