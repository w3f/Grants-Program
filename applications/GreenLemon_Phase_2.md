...
The Green Lemon Protocol is an anonymous NFT platform based on zero-knowledge proof and dual-key stealth address protocol. Anyone can deploy their anonymous NFT smart contract and integrate it with our relayer service. Or use our template NFT contract as well.

Quick facts:

* Send anonymous transactions based on zero-knowledge proof
* Encrypt the asset owner based on the dual-key stealth address protocol

### Project Details

Green Lemon Protocol is a service that allows users to obfuscate the origin and destination of transactions, and submit transactions to third contracts anonymously. Because of the logic of the blockchain, every transaction is public. If you have some DOT on your account, you cannot transfer it anonymously, because anybody can follow your transaction history on the blockchain. Green Lemon Protocol can solve this privacy problem by breaking the on-chain link between the source and the destination address by using ZKP. Most importantly, we can trigger the functions of NFT contract when we submit the zero-knowledge proof, and pay transaction fees to the relayer service. If the transaction fees you paid can cover the cost of calling a third-party contract. Then the whole logic makes sense.
...
### Work Flow
![workflow.jpg](https://raw.githubusercontent.com/GreenLemonProtocol/ink/main/docs/workflow.jpeg)
...
### Milestone 1 — Add MiMCSponge Hash and verify zero-knowledge proof on-chain into the Contract Pallet
...
### Milestone 2 — Mini Polkadot wallet (Google Chrome Extention)
...
| 0b. | Documentation | We will provide both **inline documentation** of the code and a basic **tutorial** that explains how a user can (for example) spin up one of our Substrate nodes and send test transactions, which will show how the new functionality works. |
| 0c. | Testing Guide | Core functions will be fully covered by unit tests to ensure functionality and robustness. In the guide, we will describe how to run these tests. |
| 0d. | Article | We will publish **article & demo video** that explains How is wallet works for DApp developers.
| 1. | Mini Polkadot wallet (Google Chrome Extention) | Development and testing of the mini wallet for Google Chrome. Including creating public/private key pairs, and sending the transaction to the Polkadot API node by clicking a website button. And also support the user to switch network or configure it manually. This is a `mini` wallet for Polkadot contract developers. Especially for the DApp that needs to interact through the website, our wallet provides a quick debugging feature. The first application scenario of the wallet is to use it to sign transactions in Green Lemon's NFT PC browser. we found that there is not a wallet that is suitable for the local Substrate node + local PC website to debug the contract quickly. We believe our wallet will reduce the development cost for the Dapp developers.
