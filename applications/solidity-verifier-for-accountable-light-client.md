...
So, we are looking for a better solution to the pubkeys problem of aggregate signatures. We put our eyes on zero-knowledge proofs. Zero-knowledge proofs are considered as an honest-computation proof method, so we just need to put the computation of the verification process off the chain. But through our studing, we found that the number of constraints under the generic snark scheme was so huge that not only the proof was slow to generate, but also required very powerful device. We were stuck here until we came across this paper from W3F.
