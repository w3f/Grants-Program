...
| **0b.** | Documentation               | We will provide both **inline documentation** of the code and a basic **tutorial** that explains how a user can (for example) spin up one of our hyperdot-node. |
| **0c.** | Testing and Testing Guide   | Comprehensive unit tests will be conducted to ensure the robustness of the core components: hyperdot-streaming and hyperdot-storage. <br />The testing guide will explain how to run these tests. |
| **0d.** | Build System | To enhance system deployment and provide developers with a better development experience, the following will be done:<br />1. Provide a Makefile to compile the hyperdot-streaming, hyperdot-storage, and hyperdot-ctl binary programs.<br />2. Provide a Rust build script to compile the codegen required by the hyperdot runtime.<br />3. Provide documentation for the how to build. |
| **0e.** | Docker | We will provide Dockerfiles to build Docker images for hyperdot-streaming, hyperdot-storage, and hyperdot-ctl, which can be used for running tests. |
...
| ------: | ------------------------------ | ------------------------------------------------------------ |
| **0a.** | License                        | Apache 2.0                                                   |
| **0b.** | Documentation                  | We will provide both **inline documentation** of the code and a basic **tutorial** that explains how a user can (for example) spin up one of our hyperdot-fronted-end |
| **0c.** | Testing and Testing Guide      | Comprehensive unit tests will be conducted to ensure the robustness of the core components. <br />The testing guide will explain how to run these tests. |
...
|  **3.** | My Creation - Queries Page     | My Creation Pages will showcase users' past data analysis query creations:<br/><br/>1. It will display the query name, creator, creation time, and the number of times the creation has been favorited.<br/>2. A search input will be provided for users to search for their own creations. |
|  **4.** | Favorites - Queries            | Favorites Queries will showcase users' past favorite data analysis query creations from other users, including:<br/><br/>1. It will display the creator of the data analysis query, creation time, and the number of times the creation has been favorited by other users.<br/><br/>2. A search input will be provided for users to search for their favorite queries by name.<br/><br/>3. Each query that a user has favorited can be clicked to access the details. The details page will explain the SQL syntax in the Playground Editor and display relevant visualized data tables. |
|  **5.** | Discovery - Queries            | 1. The Browse Queries section will showcase data analysis queries created by users in the community, sorted by the number of favorites. Each creation can be clicked to access the details, where the Playground Editor will explain the SQL syntax and display relevant visualized data tables. <br /><br />2. The Browse Queries section provides more comprehensive search functionality, including searching by name, sorting by the number of favorites, and searching by tags. |
|  **6.** | User                           | We will provide user management functionality, including email-based authentication and authentication pages for GitHub third-party login, as well as a logout page. |
|  **7.** | Guided Dashboards              | The Guided Dashboard Page will provide useful and common data analysis query statements and visual dashboards. Users can refer to these dashboards to copy query statements for their own creations. |
...
|      5. | Integration Testing                                  | We will perform integration testing by using test networks and local substrate along with all the components from Milestones 1 and 2 to validate the functionality of our system. |
|      6. | Ergonomics                                           | 1. We will provide a `--standalone` option in hyperdot-ctl to allow users to directly start a local hyperdot node and simplify the complexity of using hyperdot.<br />2. We will provide a Docker Compose to facilitate the deployment of the complete hyperdot system. |
|      7. | Documentation                                        | We will create documentation for the deliverables from Milestones 1 and 2. Additionally, we will establish a documentation using GitBook for the entire system, which will:<br />1. Incorporate the articles from Milestones 1 and 2 as the welcome section of the documentation, introducing what hyperdot is.<br />2. Provide a quickstart guide to help users get started quickly.<br />3. Explain the data models in the system and how to execute queries through the documentation. |
