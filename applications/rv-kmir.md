...
*Full-time engineers: 1.5*
*Overall Cost: 150,000 USD*
*License: BSD-3*
...
### Investigate existing MIR tooling (3 weeks)

Look into the existing tooling for emitting and consuming MIR definitions from the Rust compiler.
We need to be able to:
- Extract MIR from the Rust compiler before it is compiled further.
  The extracted MIR needs to be machine-readable (either in AST form or Text form).
- Have an external tool for parsing and validating MIR definitions (either the AST form or Text form).
  It appears that [**miri**](https://github.com/rust-lang/miri) may be suitable for this task.
...
- Write MIR code snippets which exercise each syntactic/semantic feature of the MIR language.
- Set up the testing harness to call **miri** on each program.
  Generate expected output for each program in the test-suite.
- Set up **kmir** executable stub for running the KMIR semantics.
  Have it fail everything initially, but handles being called in the same way as **miri** for testing/comparison.

### Develop syntax (3 weeks)

Specify the syntax of MIR programs in K.
Ensure we can parse all the conformance tests developed in the previous step.

### Develop semantics (8 weeks): **TIMEBOXED**
...
### Documentation/Announcement (2 weeks)
