...
| 5. | Contracts | Create the trait definition for a Composable Access Rule and develop a `limited use rule`, allowing a token associated with an asset id to be used only 'n' times and a `minimum balance rule` which allows a data owner to specify the minimum number of assets an address must hold to access data associated with an asset class. Additionally, we develop the RuleExecutor contract which aggregates rules and submits the results on chain. |  
