...
- **Full-Time Equivalent (FTE):**  5 employees (2 developers, 2 designers, 1 blockchain architect [Orrin])
...
| 3. | Substrate module: Z | We will create a Substrate module that will allow users to purchase, mint and transfer digital collectibles from our NFT marketplace which we will implement on the Polkadot blockchain
| 4. | Substrate chain | Modules X, Y & Z of our custom chain will interact in such a way to create a unique on-boarding experience for first time users to generate a DOT wallet [Y], connect their wallet once created [Y] and then mint NFTs on the Polkadot blockchain - transferring them to their newly created (or pre-existing) wallet(s) if they so please [Z]. 

Upon completion of questionnaire, users have the option to connect create or connect a pre-existing DOT wallet [Y] and confirm their completion on-chain as a blockchain transaction “timestamp” [X]
