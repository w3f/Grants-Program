...
| 1. | Substrate module: `settpay_serp` (0.4 BTC - 2 weeks)| The `settpay_serp` will be the module to receive the newly minted `SETT` for `SettPay` discounts & cashback distribution. It will work like this: the `settpay_serp` will receive `75%` of the newly minted `SETT` from the `serp_tes` and will distribute it to `settpay_slots`. `settpay_slots` are `SettPayX` (SettPay's payment gateway) vendors that are registered on the `SettPayX` platform. The `settpay_serp` will store these newly minted `SETT` in the `settpay_slots` where the slots can not move these funds, every slot receives equal `SETT` just like in the NPoS implementation in Polkadot, so the slots will have equal distribution of `SETT` amongst them, then the slots will be able to allocate the `SETT` to any products they want however they want in their stores or apps. The `settpay_serp` will burn `unspent_sett` from `settpay_slots` that have surpassed 30 days. The SETT slot allocation will come later in a future module called `settpay_alloc` where slots can allocate how and in which sett currency is their `sett_distro` allocated (`sett_distro` is the distributed `SETT`, the macro that tells how much `SETT` is distributed, and how much of it to which `slot`, and how much is remaining in which `slot`), the `settpay_alloc` module will be implemented in the `SettPayX` DApp. |  
| 2. | Substrate module: `serp_staking` (0.4 BTC - 2 weeks) | The `serp_staking` module will be built from the staking module and it will be Setheum's staking system. It will allow staking of `DNAR`, the serp_staking will be the module to receive `DNAR` for staking and it will lock that `DNAR` in the `dinar_stake`, then burn `25%` of it to mint `SETT` of that value and lock the `SETT` in the `sett_stake`. Then the `staking_rewards` will be increased by `25%`, this is how the initial `SETT` will be minted, the `serp_market` module will trade `DNAR-SETT` minting and contraction with the `serp_staking` module's `sett_stake` and will contract or supply relative to the amount of contraction or supply needed, it will only trade with the sett_stake while the staking_rewards increase whenever `DNAR` is contracted, then `staking_reward` decreases to normalise whenever `DNAR` is minted to the `dinar_stake`. The `SETT` in one's `sett_stake` will then be burnt for `DNAR` when unbonding stake. The `serp_market` module above in Milestone 1 explains how the `serp_trade` takes place and at what price a `serp_trade` takes place. |  
