...
| 1. | Substrate module: orml-tokens | We will use the orml pallet for the multi-token trait |
| 2. | Substrate module: orml-currencies | We will use the orml-currencies pallet to create a native currency (DEN) and additional on-chain assets (initially USD) |
...
| 0c. | Testing Guide | The code will have proper unit-test coverage (>90%) to ensure functionality and robustness. In the guide we will describe how to run these tests | 
| 1. | Substrate module: orml-oracle | We will implement an oracle which pulls market price data for calculations |  
